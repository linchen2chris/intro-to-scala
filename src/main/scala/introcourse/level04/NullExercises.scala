...
object NullExercises {
  ...
  /**
    ...
    * > null
    **/
  def mkPersonOrNull(name: String, age: Int): Person = if(name == "" || age < 0) null else Person(name, age)
  ...
  def mkPersonOrNullThenChangeName(oldName: String, age: Int, newName: String): Person = {
    ...
    if(person == null || newName == "") null else changeName(newName, mkPersonOrNull(oldName, age))
  }
